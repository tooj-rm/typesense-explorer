...
const Header = () => {
  ...
  const sortedFields = columns.toSorted((a, b) => a.label.localeCompare(b.label))

  const searchableFields = sortedFields.filter((field) => field.type === 'string')
    ...
    <header className="px-4 py-6 border-b border-border bg-background/60">
      ...
      <div className="grid gap-2 mt-4 md:grid-cols-4">
        ...
        <div className="w-full">
          ...
          <QueryByInput
            placeholder="filter_by e.g price:>50"
            fields={sortedFields}
            value={filterBy}
